...
#  https://github.com/juliarevord/259-tidying-automation-homework
...
#delete all of the data in the file
ds_loop = ds_loop %>% filter(FALSE)

for (i in paths) {
  ds_temp = read_csv(i)
  ds_loop = bind_rows (ds_loop, ds_temp)
}
...
ds_map <- map_dfr(paths,read_csv)  %>% mutate()
...
ds = ds_map 
ds_longer = pivot_longer(ds, cols = c("Male","Female"), names_to = "Sex", values_to = "Number"  )
View(ds_longer)
### Question 5 ----------
...
ds_words = left_join(ds_longer, total_words, by = "Film", copy = TRUE)
ds_words$WordPercent = (ds_words$Number/ds_words$Total)*100
...
words_graph <- function(df) {
  p <- ggplot(df, aes(x = Race, y = Number, fill = Sex)) + 
...
#       elf_percent, elf_words, 
# Male
# Female 

ds_wider = pivot_wider (ds_words, names_from = "Race", values_from = c("WordPercent", "Number"))

write.csv(ds_wider[1:2,],file="ds_wider_FellowshipOfTheRing.csv")
write.csv(ds_wider[3:4,],file="ds_wider_TwoTowers.csv")
write.csv(ds_wider[4:5,],file="ds_wider_ReturnOfTheKing.csv")
